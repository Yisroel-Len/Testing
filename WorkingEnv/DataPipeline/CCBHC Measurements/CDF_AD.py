...
class _Sub_1 (Submeasure):
    ...
    def __get_screenings(self) -> pd.DataFrame:
        ...
        # I use the PHQ-9 because it is also used in the DEP-REM-6
        sql = '''
        SELECT
            ...
            tblAssessmentToolsPHQuetions.TotalScore
        FROM 
            tblAssessmentToolsPHQuetions
        INNER JOIN
            ptPatient_Activity ON (tblAssessmentToolsPHQuetions.PatientId = ptPatient_Activity.PatientID)
        LEFT JOIN
    ...
    def return_final_data(self) -> tuple[pd.DataFrame,pd.DataFrame]:
        '''Returns the final calculated data for the CDF-AD measurement \n
        Returns: \n
...
class CDF_AD(Measurement):
    """
    Percentage of beneficiaries [clients] age 18 and older screened for depression on the date of the
